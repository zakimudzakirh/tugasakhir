...
class Data_Factory():
    ...
    def preprocess(self, path_rating, path_itemtext, min_rating,
        ...
        rating = []
        dataall = []
        userfilter = open('user_filter.txt', 'w')
        itemfilter = open('item_filter.txt', 'w')
        ...
        for line in all_line:
            ...
            if u in userset and i in itemset:
                ...
                dataall.append(str(u_idx)+"::"+str(i_idx)+"::"+tmp[2])
        raw_ratings.close()
        #print(rating)
        datasetall = open('dataset/datasetall.txt', 'w')
        userfilter.write(str(userset))
        itemfilter.write(str(itemset))
        datasetall.write(str(dataall))
        #print(dataall)
        # print userset
        # print rating
        ...
        for i in range(R.shape[1]):
            X_sequence.append(
                [vocab[word] + 1 for word in map_idtoplot[i].split() if word in vocab]) #vocab.has_key(word)])
